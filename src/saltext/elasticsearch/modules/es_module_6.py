"""
Connection library for Elasticsearch
"""
...
class ESModule6:
    ...
    def _set_elastic(self, hosts=None, profile=None):
        self.elastic = self._get_instance(hosts=hosts, profile=profile)

    def set_salt_globals(self, salt_globals):
        self.__salt__ = salt_globals

    def set_profile(self, profile):
        self.profile = profile

    def _get_instance(self, hosts=None, profile=None):
        """
        ...
        try:
            if proxies:
                ...
                elastic = self.elasticsearch.Elasticsearch(
                    hosts,
                    ca_certs=ca_certs,
            ...
            raise CommandExecutionError(
                f"Could not connect to Elasticsearch host/ cluster {hosts} due to {str(err)}"
            )
        return elastic
    ...
    def info(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.info(
                error_trace=error_trace, filter_path=filter_path, human=human, pretty=pretty
            )
        except self.elasticsearch.exceptions.TransportError as err:
            raise CommandExecutionError(
                f"Cannot retrieve server information, server returned errors {str(err)}"
            ) from err
    ...
    def node_info(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.nodes.info(
                node_id=node_id,
                metric=metric,
            ...
            raise CommandExecutionError(
                f"Cannot retrieve node information, server returned errors {str(err)}"
            ) from err
    ...
    def cluster_health(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.cluster.health(
                index=index,
                error_trace=error_trace,
            ...
            raise CommandExecutionError(
                f"Cannot retrieve health information, server returned errors {str(err)}"
            ) from err
    ...
    def cluster_allocation_explain(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.cluster.allocation_explain(
            ...
            raise CommandExecutionError(
                f"Cannot retrieve cluster allocation explanation, server returned errors {str(err)}"
            ) from err
    ...
    def cluster_pending_tasks(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.cluster.pending_tasks(
                error_trace=error_trace,
                filter_path=filter_path,
                human=human,
                local=local,
    ...
    def cluster_stats(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.cluster.stats(
                node_id=node_id,
                error_trace=error_trace,
                filter_path=filter_path,
                flat_settings=flat_settings,
            ...
            raise CommandExecutionError(
                f"Cannot retrieve cluster stats, server returned errors {str(err)}"
            ) from err
    ...
    def cluster_get_settings(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.cluster.get_settings(
                error_trace=error_trace,
                filter_path=filter_path,
                flat_settings=flat_settings,
                human=human,
            ...
            raise CommandExecutionError(
                f"Cannot retrieve cluster settings, server returned errors {str(err)}"
            ) from err
    ...
    def cluster_put_settings(
        ...
        try:
            if body is not None:
                return self.elastic.cluster.put_settings(
                    body=body,
                    flat_settings=flat_settings,
                    error_trace=error_trace,
                    filter_path=filter_path,
            ...
            raise CommandExecutionError(
                f"Cannot update cluster settings, server returned errors {str(err)}"
            ) from err
    ...
    def alias_create(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            result = self.elastic.indices.put_alias(
                index=indices,
                name=alias,
                error_trace=error_trace,
                filter=filter_,
            ...
            raise CommandExecutionError(
                f"Cannot create alias {alias} in index {indices}, server returned errors {str(err)}"
            ) from err
    ...
    def alias_delete(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            result = self.elastic.indices.delete_alias(
                index=indices,
                name=aliases,
                error_trace=error_trace,
                filter_path=filter_path,
            ...
            return result.get("acknowledged", False)
        except self.elasticsearch.exceptions.NotFoundError:
            return True
        except self.elasticsearch.exceptions.TransportError as err:
            raise CommandExecutionError(
                f"Cannot delete alias {aliases} in index {indices}, server returned errors {str(err)}"
            ) from err
    ...
    def alias_exists(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.indices.exists_alias(
                name=aliases,
                index=indices,
            ...
            raise CommandExecutionError(
                f"Cannot get alias {aliases} in index {indices}, server returned errors {str(err)}"
            ) from err
    ...
    def alias_get(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.indices.get_alias(
                index=indices,
                name=aliases,
                allow_no_indices=allow_no_indices,
                error_trace=error_trace,
    ...
    def document_create(
        ...
        if source and body:
            ...
            raise SaltInvocationError(message)
        if source:
            body = self.__salt__["cp.get_file_str"](source, saltenv=__opts__.get("saltenv", "base"))
        try:
            if body is not None:
                return self.elastic.index(
                    index=index,
                    document=body,
                    id=id_,
                    error_trace=error_trace,
            ...
            raise CommandExecutionError(
                f"Cannot create document in index {index}, server returned errors {str(err)}"
            ) from err
    ...
    def document_delete(
        ...
        try:
            ...
            raise CommandExecutionError(
                f"Cannot delete document {id} in index {index}, server returned errors {str(err)}"
            ) from err
    ...
    def document_exists(
        ...
        try:
            ...
            raise CommandExecutionError(
                f"Cannot retrieve document {id} from index {index}, server returned errors {str(err)}"
            ) from err
    ...
    def index_create(
        ...
        src_map = None
        if source:
            src_map = self.__salt__["cp.get_file_str"](
        ...
        try:
            ...
            result = self.elastic.indices.create(
                index=index,
                aliases=aliases,
                error_trace=error_trace,
                filter_path=filter_path,
            ...
            if "index_already_exists_exception" in str(err):
                return True
            raise CommandExecutionError(
                f"Cannot create index {index}, server returned errors {str(err)}"
            ) from err
    ...
    def index_delete(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            result = self.elastic.indices.delete(
                index=index,
                allow_no_indices=allow_no_indices,
                error_trace=error_trace,
                expand_wildcards=expand_wildcards,
            ...
            raise CommandExecutionError(
                f"Cannot delete index {index}, server returned errors {str(err)}"
            ) from err
    ...
    def index_exists(
        ...
        try:
            ...
            raise CommandExecutionError(
                f"Cannot retrieve index {index}, server returned errors {str(err)}"
            ) from err
    ...
    def index_get(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.indices.get(
                index=index,
                allow_no_indices=allow_no_indices,
                error_trace=error_trace,
                expand_wildcards=expand_wildcards,
    ...
    def index_open(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            result = self.elastic.indices.open(
                index=index,
                allow_no_indices=allow_no_indices,
                expand_wildcards=expand_wildcards,
                ignore_unavailable=ignore_unavailable,
            ...
            raise CommandExecutionError(
                f"Cannot open index {index}, server returned errors {str(err)}"
            ) from err
    ...
    def index_close(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            result = self.elastic.indices.close(
                index=index,
                allow_no_indices=allow_no_indices,
                expand_wildcards=expand_wildcards,
                ignore_unavailable=ignore_unavailable,
            ...
            raise CommandExecutionError(
                f"Cannot close index {index}, server returned errors {str(err)}"
            ) from err
    ...
    def index_get_settings(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.indices.get_settings(
                index=index,
                name=name,
                allow_no_indices=allow_no_indices,
                error_trace=error_trace,
            ...
            raise CommandExecutionError(
                f"Cannot retrieve index settings, server returned errors {str(err)}"
            ) from err
    ...
    def index_put_settings(
        ...
        try:
            result = self.elastic.indices.put_settings(
                settings=settings,
                index=index,
                allow_no_indices=allow_no_indices,
                error_trace=error_trace,
            ...
            raise CommandExecutionError(
                f"Cannot update index settings, server returned errors {str(err)}"
            ) from err
    ...
    def mapping_create(
        ...
        try:
            result = self.elastic.indices.put_mapping(
                index=index,
                allow_no_indices=allow_no_indices,
            ...
            raise CommandExecutionError(
                f"Cannot create mapping {index}, server returned errors {str(err)}"
            ) from err
    ...
    def mapping_get(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.indices.get_mapping(
                index=index,
                allow_no_indices=allow_no_indices,
                error_trace=error_trace,
                expand_wildcards=expand_wildcards,
            ...
            raise CommandExecutionError(
                f"Cannot retrieve mapping {index}, server returned errors {str(err)}"
            ) from err
    ...
    def index_template_create(
        ...
        try:
            result = self.elastic.indices.put_template(
                name=name,
                aliases=aliases,
                create=create,
                error_trace=error_trace,
            ...
            raise CommandExecutionError(
                f"Cannot create template {name}, server returned errors {str(err)}"
            ) from err
    ...
    def index_template_delete(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            result = self.elastic.indices.delete_template(
                name=name,
                error_trace=error_trace,
                filter_path=filter_path,
                human=human,
            ...
            raise CommandExecutionError(
                f"Cannot delete template {name}, server returned errors {str(err)}"
            ) from err
    ...
    def index_template_exists(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.indices.exists_index_template(
                name=name,
                error_trace=error_trace,
                filter_path=filter_path,
                human=human,
            ...
            raise CommandExecutionError(
                f"Cannot retrieve template {name}, server returned errors {str(err)}"
            ) from err
    ...
    def template_exists(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.indices.exists_template(
                name=name,
                error_trace=error_trace,
                filter_path=filter_path,
                flat_settings=flat_settings,
    ...
    def index_template_get(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.indices.get_template(
                name=name,
                error_trace=error_trace,
                filter_path=filter_path,
                flat_settings=flat_settings,
    ...
    def geo_ip_stats(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.ingest.geo_ip_stats(
                error_trace=error_trace,
                filter_path=filter_path,
                human=human,
                pretty=pretty,
            ...
            raise CommandExecutionError(
                f"Cannot get geo_ip_stats, server returned errors {str(err)}"
            ) from err
    ...
    def processor_grok(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.ingest.processor_grok(
                error_trace=error_trace,
                filter_path=filter_path,
                human=human,
                pretty=pretty,
            ...
            raise CommandExecutionError(
                f"Cannot get built-in patterns, server returned errors {str(err)}"
            ) from err
    ...
    def pipeline_get(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.ingest.get_pipeline(
                id=id_,
                error_trace=error_trace,
                filter_path=filter_path,
                human=human,
            ...
            raise CommandExecutionError(
                f"Cannot create pipeline {id}, server returned errors {str(err)}"
            ) from err
    ...
    def pipeline_delete(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            ret = self.elastic.ingest.delete_pipeline(
                id=id_,
                error_trace=error_trace,
                filter_path=filter_path,
                human=human,
            ...
            raise CommandExecutionError(
                f"Cannot delete pipeline {id}, server returned errors {str(err)}"
            ) from err
        except AttributeError as err:
            raise CommandExecutionError("Method is applicable only for Elasticsearch 5.0+") from err
    ...
    def pipeline_create(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            out = self.elastic.ingest.put_pipeline(
                id=id_,
    ...
    def pipeline_simulate(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            result = self.elastic.ingest.simulate(
                id=id_,
                docs=docs,
                error_trace=error_trace,
                filter_path=filter_path,
            ...
            raise CommandExecutionError(
                f"Cannot simulate pipeline {id}, server returned errors {str(err)}"
            ) from err
        except AttributeError as err:
            raise CommandExecutionError("Method is applicable only for Elasticsearch 5.0+") from err
    ...
    def script_get(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.get_script(
                id=id_,
                error_trace=error_trace,
                filter_path=filter_path,
                human=human,
            ...
            raise CommandExecutionError(
                f"Cannot obtain search template {id}, server returned errors {str(err)}"
            ) from err
    ...
    def script_create(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            result = self.elastic.put_script(
                id=id_,
                script=script,
                context=context,
                error_trace=error_trace,
            ...
            raise CommandExecutionError(
                f"Cannot create search template {id}, server returned errors {str(err)}"
            ) from err
    ...
    def script_delete(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            result = self.elastic.delete_script(
                id=id_,
                error_trace=error_trace,
                filter_path=filter_path,
                human=human,
            ...
            raise CommandExecutionError(
                f"Cannot delete search template {id}, server returned errors {str(err)}"
            ) from err
    ...
    def repository_get(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.snapshot.get_repository(
                name=name,
                local=local,
                error_trace=error_trace,
                filter_path=filter_path,
            ...
            raise CommandExecutionError(
                f"Cannot obtain repository {name}, server returned errors {str(err)}"
            ) from err
    ...
    def repository_cleanup(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.snapshot.cleanup_repository(
                name=name,
                error_trace=error_trace,
                filter_path=filter_path,
                human=human,
    ...
    def repository_create(
        ...
        try:
            if body is not None:
                result = self.elastic.snapshot.create_repository(
                    name=name, type=type_, settings=body
                )
            else:
                result = self.elastic.snapshot.create_repository(
                    name=name,
            ...
            raise CommandExecutionError(
                f"Cannot create repository {name}, server returned errors {str(err)}"
            ) from err
    ...
    def repository_delete(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            result = self.elastic.snapshot.delete_repository(
                name=name,
                error_trace=error_trace,
                filter_path=filter_path,
                human=human,
            ...
            raise CommandExecutionError(
                f"Cannot delete repository {name}, server returned errors {str(err)}"
            ) from err
    ...
    def repository_verify(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.snapshot.verify_repository(
                name=name,
                error_trace=error_trace,
                filter_path=filter_path,
                human=human,
            ...
            raise CommandExecutionError(
                f"Cannot verify repository {name}, server returned errors {str(err)}"
            ) from err
    ...
    def snapshot_status(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.snapshot.status(
                repository=repository,
                snapshot=snapshot,
                ignore_unavailable=ignore_unavailable,
                error_trace=error_trace,
            ...
            raise CommandExecutionError(
                f"Cannot obtain snapshot status, server returned errors {str(err)}"
            ) from err
    ...
    def snapshot_clone(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            result = self.elastic.snapshot.clone(
                repository=repository,
                indices=indices,
                snapshot=snapshot,
    ...
    def snapshot_get(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            return self.elastic.snapshot.get(
                repository=repository,
                snapshot=snapshot,
                ignore_unavailable=ignore_unavailable,
                after=after,
            ...
            raise CommandExecutionError(
                ...
                f"server returned errors {str(err)}"
            ) from err
    ...
    def snapshot_create(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            response = self.elastic.snapshot.create(
                repository=repository,
                snapshot=snapshot,
                error_trace=error_trace,
            ...
            raise CommandExecutionError(
                f"Cannot create snapshot {snapshot} in repository {repository}, server returned errors {str(err)}"
            ) from err
    ...
    def snapshot_restore(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            response = self.elastic.snapshot.restore(
                repository=repository,
                snapshot=snapshot,
                error_trace=error_trace,
                filter_path=filter_path,
            ...
            raise CommandExecutionError(
                f"Cannot restore snapshot {snapshot} in repository {repository}, server returned errors {str(err)}"
            ) from err
    ...
    def snapshot_delete(
        ...
        self._set_elastic(hosts=hosts, profile=profile)
        try:
            result = self.elastic.snapshot.delete(
                repository=repository,
                snapshot=snapshot,
                error_trace=error_trace,
                filter_path=filter_path,
            ...
            raise CommandExecutionError(
                f"Cannot delete snapshot {snapshot} from repository {repository}, server returned errors {str(err)}"
            ) from err
